...
export const register: Router.Middleware<
  DefaultState,
  AppContext<RegisterResponseDto, RegisterRequestModel>
> = async (ctx) => {
  ctx.body = await authService.register(ctx.validatedRequest.value);
};

export const login: Router.Middleware<DefaultState, AppContext<LoginResponseDto, LoginRequestModel>> = async (
  ctx,
) => {
